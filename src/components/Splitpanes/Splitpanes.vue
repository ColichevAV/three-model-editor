...
export default defineComponent({
    ...
    props: {
        panesStateCount: { type: Number, requied: true },
    ...
    provide() {
        return {
            ...
            onPaneUpdate: this.onPaneUpdate,
    ...
    methods: {
        ...
        requestUpdate({ target, ...args }) {
            ...
            if (pane) Object.entries(args).forEach(([key, value]) => (pane[key] = value))
        },
        ...
        onPaneAdd(pane) {
            ...
            //console.log("paneAdded: " + this.panesCount + " - " + this.panesStateCount);

            if (this.panesStateCount == this.panesCount) this.recalculatePaneSizes2()
        },
        onPaneUpdate(pane) {
            //console.log("onPaneUpdate");
            // 1. Add pane to array at the same index it was inserted in the <splitpanes> tag.
            let index = -1
            Array.from(pane.$el.parentNode.children).some((el) => {
                if (el.className.includes('splitpanes__pane')) index++
            ...
            this.recalculatePaneSizes2()
        },
        onPaneRemove(pane) {
            console.log('onPaneRemove')
        ...
        resetPaneSizes(changedPanes = {}) {
            ...
            //	this.recalculatePaneSizes2(changedPanes);
        ...
        // 	if (leftToAllocate > 0.1) this.readjustSizes(leftToAllocate, ungrowable, unshrinkable);
        // },
        recalculatePaneSizes2() {
            let dynPanes = this.panes.filter((x) => x.givenSize === null)
            let staticPanes = this.panes.filter((x) => x.givenSize !== null)
            let lastPane = this.panes[this.panes.length - 1]

            var staticW = 0
            if (staticPanes.length > 0)
                staticW =
                    staticPanes.length > 1 ? sumBy(staticPanes, (x) => x.size) : staticPanes[0].size

            // console.log("static: " + staticW);
            // console.log(staticW);
            // console.log(this.panes);
            // console.log(staticPanes);
            // console.log(dynPanes);

            const dynamicW = 100 - staticW

            if (dynPanes.length > 0) {
                dynPanes.forEach((x) => (x.size = dynamicW / dynPanes.length))
            } else if (lastPane) {
                lastPane.size += dynamicW
            }
        },
...
.splitpanes {
    ...
    &__pane {
        ...
        .splitpanes--vertical & {
            transition: none; //width 0.2s ease-out;
        }

        .splitpanes--horizontal & {
            transition: none; //height 0.2s ease-out;
        }
